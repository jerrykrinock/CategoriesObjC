...
/*!
 ...
 Tells the invocation to retain its arguments, but note that this will not
 retain all argument types.&nbsp; See the 10.5 Cocoa Foundation release
 notes http://developer.apple.com/releasenotes/Cocoa/Foundation.html and search for
 text 'retainArguments'.&nbsp;  Since this link may break or be different when
 10.6 is released, I've copied all the text and pasted it in below.&nbsp;  Enjoy!!
...
/*!
 ...
 ignores any further arguments.&nbsp; A NULL sentinel termination is not required.
 
 However, the flip side of this is that you must provide as many arguments in
 argumentAddresses as the method signature of 'selector' requires.  If an argument
 is not known yet, pass a NULL placeholder.  The reason is that these
 arguments will be 'set' into the invocation and thus may be sent messages if the
 method signature indicates that they are objects.
 
 If, after creating an invocation using this method, you want to modify an argument,
 you can do so by invoking -setArgument:atIndex:.&nbsp; But when doing so, remember
 that argument indexes start at 2.&nbsp;  For example, to modify the first argument,
 pass atIndex:2.
 
 Also remember that, per NSInvocation documentation, none of the the parameters of
 the selector being invoked may themselves be a va_arg argument list, nor a union.
